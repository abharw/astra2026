...
The extension's physical iPhone build passed. The first installation attempt lost the device connection; after reconnection, installation and launch succeeded. A fresh ARView snapshot from the installed extension then succeeded with a camera-frame age of about 0.05 seconds. It showed only a close-up gray surface while saved-room relocalization was in progress, so object selection, drag and question acceptance remain pending camera repositioning. The previous camera-link build had already installed and returned a real camera image. Relay tests also reject out-of-range drags and empty questions. Do not describe the untested extension as a completed physical drag or question demonstration.
