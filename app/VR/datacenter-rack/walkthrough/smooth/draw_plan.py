...
d.text((40,25),'Data-center maze: ground truth',font=font(35),fill='#192735');d.text((42,75),'Straight → left → right → left • 21.94 m rounded route • 30-second regular POV video',font=font(21),fill='#55616b')
...
route=[pt(*v['position_m'][:2]) for v in data['camera']['poses'][::3]];d.line(route,fill='#087f6c',width=7)
for i,(x,y) in enumerate(route):
 d.ellipse((x-8,y-8,x+8,y+8),fill='#f4f3ee',outline='#087f6c',width=3)
 if False:d.text((x+12,y+10),str(i),font=font(19),fill='#087f6c')
for i in range(0,len(route)-1,30):
...
for lm,c in zip(data['landmarks'],['#707879']*4):
...
lines=['CAMERA ROUTE','','0–23.5 s   Continuous walking','Rounded left, right, left turns','Gentle anticipation into each bend','23.5–30 s   Slow look-around','','ORDINARY FIXED FEATURES','','Gray unlabelled utility panels','Plain gray maintenance door','Server racks and cable trays','','SETTINGS','','1.65 m eye height; 24 mm lens','24 fps; continuous motion; no cuts','1.6 m corner radius','Maximum pan rate: 38.7 degrees/s','','TEST BOUNDARY','','Supply only the video to the mapper.','Keep this plan and camera poses','separate for checking its answer.']
