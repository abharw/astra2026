...
CONFIG = ROOT / ".local" / "dev-session.json"
...
def diagnostics_destination(target):
    ...
    return ROOT / ".local" / "diagnostics" / f"{timestamp}-{safe_target}"
...
def main():
    ...
    if args.command == "serve":
        ...
        token = existing["token"] if existing and not args.rotate_token else secrets.token_urlsafe(32)
        config = {"url": url, "token": token}
        if config != existing:
            write_config(config)
        env = os.environ.copy()
        log_dir = ROOT / ".local" / "logs" / f"backend-{args.port}"
        ...
        print(f"Development service: {config['url']} (session token kept in ignored .local directory)", flush=True)
