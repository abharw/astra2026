...
const samples = ["framework/contract/fixtures/accepted/imported_rack_document.json", "assets/server-rack/scene.json"].map((path) => {
...
const imported = JSON.parse(readFileSync(new URL("framework/contract/fixtures/accepted/imported_rack_document.json", root), "utf8")) as JsonObject;
...
const sourceFiles = ["backend/src/astra/scene-context.ts", "backend/src/astra/authoring-tool.ts", "backend/src/astra/instructions.ts", "backend/src/astra/conversation-context.ts", "backend/src/astra/client.ts", "backend/src/normalizer.ts", "backend/src/session.ts", "backend/src/diagnostics.ts"];
const evidence = {
  schema: "astra-model-context-optimization/v1",
  measuredAt: new Date().toISOString(),
  reproduce: "npm --prefix backend run check:context",
...
const destination = new URL("docs/evidence/model-context-optimization.json", root);
