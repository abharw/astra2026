...
Usage: python3 tools/checks/check-imported-rack-native.py [--variant conservative]
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=root / "docs/evidence/imported-rack-native.json")
    args = parser.parse_args()
    manifest_path = root / "assets/imported-rack/native-validation-catalogs.json"
    ...
    with tempfile.TemporaryDirectory(prefix="astra-native-import-") as temporary:
        ...
        for item in variants:
            ...
            command = ["swift", "test", "--package-path", "framework",
                       "--scratch-path", ".local/build/framework", "--filter",
    ...
    tracked_sources = [
        "framework/Sources/SpatialApple/Rendering/ImportedAsset.swift",
        "framework/Sources/SpatialApple/Rendering/SceneRenderer.swift",
        "framework/Sources/SpatialApple/SceneController.swift",
        "framework/Tests/SpatialAppleTests/ImportedAssetTests.swift",
    ]
    evidence = {
        "schema": "astra-imported-rack-native-evidence/v1",
        "status": "passed",
        "reproduce": "python3 tools/checks/check-imported-rack-native.py",
