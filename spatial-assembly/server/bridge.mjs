...
const tools = [
 ...
 {type:'function',name:'manipulate',description:'Manipulate the active assembly. Pull out with extract before moving, rotating or scaling. Amount: explode 0..1; movement metres; rotate degrees; scale multiplier.',parameters:properties({action:{type:'string',enum:actions},amount:{type:'number'},part:{type:'string'}})}
];
export function startBridge({key,token,port=8796,host='127.0.0.1',model='gpt-6-astra',voiceModel='gpt-realtime-2.1',request,log=()=>{}}) {
 ...
 wss.on('connection',client=>{
  const sessionId=randomUUID();let upstream=null,voiceReady=false,job=null,latestScene=null,selectedPart='',activeObject='',viewRequest=null,requestCount=0;
  ...
  const sceneContext=()=> latestScene ? {name:latestScene.name,selectedPart,parts:latestScene.parts.map(p=>({id:p.id,name:p.name,description:p.description,function:p.function,evidence:p.evidence,uncertainty:p.uncertainty,sourceIds:p.sourceIds})),sources:latestScene.research?.sources || [],uncertainty:latestScene.research?.gaps || []} : null;
  ...
  client.on('message',raw=>{let e;try{e=JSON.parse(raw);}catch{return;}switch(e.type){
   ...
   case 'scene.update':
    try {latestScene=e.assembly?validateAssembly(e.assembly):null;activeObject=String(e.object_id || '');selectedPart=String(e.selected_part || '');updateVoiceScene();}catch{send('scene.error',{message:'Invalid scene state'});}break;
   case 'part.explain': {
