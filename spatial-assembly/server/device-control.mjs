...
export function startDeviceControl({deviceToken,adminToken,port=8798}){
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  if(phone?.readyState!==WebSocket.OPEN){respond(409,{error:'Enable the test connection in the active iPhone or Quest app'});return;}
  ...
  const id=randomUUID();const timer=setTimeout(()=>{pending.delete(id);respond(504,{error:'Device acknowledgment timed out'});},15000);
 ...
 wss.on('connection',socket=>{
  if(phone?.readyState===WebSocket.OPEN){socket.close(1008,'A device is already connected');return;}phone=socket;socket.send(JSON.stringify({type:'connected'}));
  ...
  socket.on('close',()=>{if(phone===socket){phone=null;for(const item of pending.values()){clearTimeout(item.timer);item.respond(409,{error:'Device disconnected'});}pending.clear();}});
 });
