...
namespace SpatialAssembly {
 public class QuestAssemblyController:MonoBehaviour {
  ...
  public Text StatusText,DetailText;public Transform Panel;
  ...
  void Update(){
   ...
   if(StatusText)StatusText.text="SPATIAL ASSEMBLY\n"+Bridge.Status+" · "+Audio.Status+"\n"+Status+(busy?$" · {(int)(Time.realtimeSinceStartup-started)}s":"")+"\n"+Store.Status;
  }
  public void PositionPanel(){if(Panel&&Rig){Panel.position=Rig.centerEyeAnchor.position+Rig.centerEyeAnchor.forward*1.2f-Vector3.up*.35f;Panel.rotation=Quaternion.LookRotation(Panel.position-Rig.centerEyeAnchor.position,Vector3.up);}}
  ...
  public void PullOrReturn(){if(!Active)return;if(Active.Extracted)Active.ReturnHome();else Active.PullOut(Rig.centerEyeAnchor);SyncScene();Store.SaveCurrent();}
  public void ExplainNext(){if(!Active)return;var p=Active.Data.parts[explainIndex++%Active.Data.parts.Count];selectedPart=p.id;Active.Select(p.id);ShowPart(p);SyncScene();Bridge.Send(new JObject{{"type","part.explain"},{"part",p.id}});}
  ...
  void OnMessage(JObject e){Audio.OnMessage(e);switch((string)e["type"]){
   ...
   case "voice.transcript":if(DetailText)DetailText.text=(string)e["text"];break;
