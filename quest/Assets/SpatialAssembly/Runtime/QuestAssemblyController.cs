...
namespace SpatialAssembly {
 public class QuestAssemblyController:MonoBehaviour {
  ...
  void Update(){
   ...
   if(OVRInput.GetDown(OVRInput.Button.Two,OVRInput.Controller.LTouch)){if(leftGrip)Cancel();else Refine();}
   ...
   if(pressed){if(part){pointedTargetLocked=false;Active=part.Owner;selectedPart=part.Part.id;Active.Select(selectedPart);ShowPart(part.Part);SyncScene();}else if(hasTarget){LockPoint(target,normal);if(!busy)Reconstruct();}}
  ...
  void UpdateStatus(){
   string state=!Bridge.Connected?"OFFLINE":!CameraAccess.IsPlaying?"CAMERA NOT READY":grabbed?"HOLDING OBJECT":busy?"RECONSTRUCTING":!string.IsNullOrEmpty(Audio.Error)?"VOICE ERROR":Audio.Speaking?"SPEAKING":Audio.Enabled?"LISTENING":Audio.Starting?"STARTING VOICE":"READY";
   string next=!Bridge.Connected?"Reconnecting automatically. Check Quest and Mac Wi-Fi.":!CameraAccess.IsPlaying?"Allow camera access in the headset.":grabbed?"Move your hand/controller. Release grip to drop; trigger places on a surface.":busy?"Captured target is marked in amber. You can inspect existing models while this runs.":!string.IsNullOrEmpty(Audio.Error)?Audio.Error+" Press B to retry voice.":Audio.Speaking?"Reply is playing. Speak to interrupt or press B to stop voice.":Audio.Enabled?"Ask a question aloud. Your microphone is on.":Active?Active.Extracted?"Grip the model to move it. While holding, trigger places it on a surface.":"Grip a model to move it. Trigger selects a part. A explodes.":hasTarget?"Point at a real object. Press trigger once to reconstruct it.":"Aim at a nearby real surface until the cyan target appears.";
