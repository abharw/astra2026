...
namespace SpatialAssembly {
 ...
 public class SavedAssemblies:MonoBehaviour {
  ...
  public async void Restore(){if(loading||records.Count==0)return;loading=true;Status="Looking for saved anchors…";try{var pending=records.Values.Where(r=>!live.Any(v=>v&&v.ObjectId==r.objectId)).ToList();var anchors=new List<OVRSpatialAnchor.UnboundAnchor>();var ids=pending.Select(r=>Guid.Parse(r.anchorId)).Distinct().ToList();var result=await OVRSpatialAnchor.LoadUnboundAnchorsAsync(ids,anchors);if(!result.Success){Status="Saved anchors unavailable; look around and Retry restore";return;}int restored=0;foreach(var unbound in anchors){if(!unbound.Localized&&!await unbound.LocalizeAsync(15))continue;foreach(var record in pending.Where(r=>r.anchorId==unbound.Uuid.ToString())){if(deleted.Contains(record.objectId)||!records.ContainsKey(record.objectId))continue;record.assembly.Validate();var root=new GameObject("Restored spatial anchor");var anchor=root.AddComponent<OVRSpatialAnchor>();unbound.BindTo(anchor);var child=new GameObject(record.assembly.name);child.transform.SetParent(root.transform,false);var v=child.AddComponent<AssemblyVisual>();v.ObjectId=record.objectId;v.Hologram=record.hologram;v.ShowInferred=record.inferred;v.Extracted=record.extracted;v.Explosion=record.explosion;v.InternalsRevealed=record.internalsRevealed;v.Build(record.assembly);v.HomePosition=AssemblyData.V(record.homePosition);v.HomeRotation=R(record.homeRotation);v.HomeScale=AssemblyData.V(record.homeScale);child.transform.localPosition=AssemblyData.V(record.position);child.transform.localRotation=R(record.rotation);child.transform.localScale=AssemblyData.V(record.scale);live.Add(v);Restored?.Invoke(v);restored++;}}Status=$"Restored {restored} · unmatched anchors stay hidden";}catch(Exception e){Status="Restore failed: "+e.Message;}finally{loading=false;}}
