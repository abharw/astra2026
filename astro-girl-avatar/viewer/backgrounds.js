...
let state=null;
export async function backgroundRequest(context,{force=false}={}){
 const r=await fetch('/api/background/context',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({context,force})});
...
const events=new EventSource('/api/background/events');
events.onmessage=e=>{
 ...
 $('background-description').textContent=state.error||(['generating','queued'].includes(state.status)?'Astra can keep speaking while the next background is made.':state.enabled?'Backgrounds follow the conversation when the topic changes.':'Automatic changes are paused. You can request a scene below.');
