...
namespace SpatialAssembly {
 public class RealtimeAudio:MonoBehaviour {
  ...
  public void OnMessage(JObject e){switch((string)e["type"]){case "voice.ready":if(!starting)return;mic=Microphone.Start(null,true,10,24000);if(!mic){Status="Microphone unavailable";Stop();return;}readPosition=0;Enabled=true;starting=false;Status="Listening";break;case "voice.audio":var data=Convert.FromBase64String((string)e["audio"]);lock(audioLock){for(int i=0;i+1<data.Length;i+=2){if(samples.Count>24000*10)samples.Dequeue();samples.Enqueue((short)(data[i]|data[i+1]<<8)/32768f);}}break;case "voice.speech_started":lock(audioLock)samples.Clear();break;case "voice.error":Stop();var message=(string)e["message"]??"Voice service failed";Error=message.Contains("API key")?"Voice authentication failed. Check the Mac bridge connection.":message;Status="Voice failed";break;case "voice.closed":case "disconnected":Stop(false);break;}}
