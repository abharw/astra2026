...
public static class BuildQuest {
 ...
 public static void Prepare(){
  ...
  var canvasGO=new GameObject("Assembly controls",typeof(RectTransform),typeof(Canvas));var canvas=canvasGO.GetComponent<Canvas>();canvas.renderMode=RenderMode.WorldSpace;var rect=canvasGO.GetComponent<RectTransform>();rect.sizeDelta=new Vector2(1100,720);rect.localScale=Vector3.one*.0007f;rect.position=new Vector3(0,1.2f,1.2f);controller.Panel=rect;
  var background=new GameObject("Panel background",typeof(RectTransform),typeof(Image));background.transform.SetParent(rect,false);var bg=background.GetComponent<RectTransform>();bg.sizeDelta=rect.sizeDelta;background.GetComponent<Image>().color=new Color(.018f,.04f,.055f,.9f);
  controller.StatusText=Text(rect,"Status",new Vector2(0,245),new Vector2(1040,200),24,"SPATIAL ASSEMBLY\nConnecting camera and bridge…");
  controller.DetailText=Text(rect,"Part explanation",new Vector2(0,-20),new Vector2(1040,300),22,"Point at a real object. Trigger / pinch to reconstruct.\nA: explode · B: voice · X: next part · Y: research & rebuild\nSelected camera images and enabled microphone audio go to OpenAI.");
  Button(rect,"Reconstruct",new Vector2(-390,-225),()=>controller.Reconstruct(),controller,"Reconstruct");
  Button(rect,"Pull / Return",new Vector2(-130,-225),()=>controller.PullOrReturn(),controller,"PullOrReturn");
  Button(rect,"Explain",new Vector2(130,-225),()=>controller.ExplainNext(),controller,"ExplainNext");
  Button(rect,"Voice",new Vector2(390,-225),()=>audio.Toggle(),audio,"Toggle");
  Button(rect,"Rebuild",new Vector2(-390,-305),()=>controller.Refine(),controller,"Refine");
  Button(rect,"Cancel",new Vector2(-130,-305),()=>controller.Cancel(),controller,"Cancel");
  Button(rect,"Restore",new Vector2(130,-305),()=>store.Restore(),store,"Restore");
  Button(rect,"Reconnect",new Vector2(390,-305),()=>bridge.Connect(),bridge,"Connect");
 ...
 static void Button(Transform parent,string label,Vector2 position,Action action,UnityEngine.Object receiver,string method){var t=Text(parent,label,position,new Vector2(240,65),24,label);t.alignment=TextAnchor.MiddleCenter;var image=new GameObject("Button background",typeof(RectTransform),typeof(Image));image.transform.SetParent(parent,false);image.transform.SetSiblingIndex(t.transform.GetSiblingIndex());image.GetComponent<RectTransform>().anchoredPosition=position;image.GetComponent<RectTransform>().sizeDelta=new Vector2(238,62);image.GetComponent<Image>().color=new Color(.02f,.28f,.36f,.95f);var collider=t.gameObject.AddComponent<BoxCollider>();collider.size=new Vector3(240,65,10);var button=t.gameObject.AddComponent<PersistentWorldButton>();button.Receiver=receiver;button.Method=method;}
