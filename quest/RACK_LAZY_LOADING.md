...
- **Bring all here** is a small button below your view, beside the microphone badge. Aim the controller ray and press the trigger, or point and pinch, to gather every current and saved object in front of you. No object selection or voice connection is required. Models reassemble, retain their sizes, and are spaced apart; the rack is selected afterward.
- Missing saved objects are recovered even if their old room anchors cannot localize. Their existing object IDs and source data are reused, with a new anchor at the recovery placement. That placement becomes home for those recovered objects; objects already restored retain their original home. A placement backup is kept before recall, and old saved records remain until the new anchor is saved. Deleted objects stay deleted.
