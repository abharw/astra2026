...
### Smooth grabbing, rotation, material colors, and empty-scene reset

The wearer requested easier bring-to-me/grab movement, rotation, normal item colors, all prior control fixes, and a complete reset. A distant model now eases into reach when grip begins; a previously selected model can be grabbed without aiming at it again. Wrist orientation rotates it; stick left/right turns, up/down adjusts distance, and left grip plus up/down tilts. Left grip + A brings the item nearby without continued dragging. Large models fit to an inspection size; Return restores their saved original position, rotation and scale. Movement has a dead zone, damped position/rotation, and a fixed input source during each grab. Refining a held model retains the grab on the replacement.
