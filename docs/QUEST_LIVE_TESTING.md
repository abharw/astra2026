...
| Panel too low | Wearer reported it near the floor; cast showed controls below the main view. A level-direction recenter change alone did not resolve the reported experience. | Default panel now follows horizontal heading at current eye height, so it updates after tracking settles and as the wearer moves. Grab/release pins it. Recenter resumes following. Live acceptance of this final adjustment is pending. |
