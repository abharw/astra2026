...
**Follow the conversation** is on by default. After every **three recognized user turns**, GPT Image 2.5 Flare chooses and generates a background from those turns. The live studio counts each final voice transcript or submitted text once by user item ID. Generation starts without waiting for Astra to finish replying, and interrupting her does not discard your turn. Empty transcripts and duplicate events do not count. The older completed-response tracker remains exported for external callers; the studio uses `createUserBackgroundTurnTracker`.

Explicit background requests bypass the wait. Say “Talk to me from a rainforest,” “Take me to Mars,” or “Change the background to a forest,” or use **Set a scene → Generate scene**. Follow-up directions such as “make it nighttime” update the requested environment; Astra also responds in that setting. Factual questions about a place do not trigger an immediate move. Explicit requests restart the three-turn counter; their acknowledgement does not count as another ordinary turn. Turn off **Follow the conversation** to pause automatic changes; direct requests still work. **Studio background** restores the original gradient and resets the counter. Voice continues while images generate. The direct Flare path uses low quality at 1024×1024 and measured 12.429 seconds in one image test, versus the earlier 23.5-second director/tool path. Timing varies; an explicit request starts generation immediately but still waits for the image service.
...
| `POST /api/background/context` | Submit one recognized user turn with `{context, turnId?}`; use `force: true` for an explicit scene request |
| `POST /api/background` | Set `{enabled: false}` or `{reset: true}` |

The standard API credential stays on the local server. This follows the [OpenAI image generation guide](https://developers.openai.com/api/docs/guides/image-generation): GPT Image 2.5 Flare called directly through `/v1/images/generations`, without a separate scene-direction model. No separate image-generation service or API key is required.
