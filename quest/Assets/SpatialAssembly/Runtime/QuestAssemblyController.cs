...
namespace SpatialAssembly {
 public class QuestAssemblyController:MonoBehaviour {
  ...
  const int MaxJobs=4;
  class ReconstructionJob {public string id,objectId,stage;public bool refine;public Snapshot capture;public float started;public int selectionVersion,number,epoch;public Transform marker;public TextMesh label;}
  ...
  public void Cancel(){generationEpoch++;foreach(var id in jobs.Keys)cancelledJobs.Add(id);if(cancelledJobs.Count>128)cancelledJobs.Clear();Bridge.Send(new JObject{{"type","reconstruction.cancel"},{"epoch",generationEpoch}});ClearJobs();Status="All reconstructions cancelled";}
  ...
  public (bool ok,string message) DeleteActive(){if(!Active)return(false,"No generated object selected");if(jobs.Values.Any(j=>j.objectId==Active.ObjectId))return(false,"Cancel this item’s reconstruction before deleting it");EndGrab();var removed=Active;string name=removed.Data.name;if(!Store.Delete(removed))return(false,Store.Status);selectionVersion++;pointedTargetLocked=false;objects.Remove(removed);Active=objects.LastOrDefault(v=>v);selectedPart=null;if(DetailText)DetailText.text="Deleted generated object: "+name;SyncScene();return(true,"Deleted generated object: "+name);}
  ...
  void Capture(string id,bool refine,string hint,Pose? inspectedPose=null,byte[] inspectedImage=null,string objectId=null){
   ...
   try{
    ...
    if(refine){objectId=objectId??(jobs.TryGetValue(id,out var pending)?pending.objectId:Active?.ObjectId);original=objects.FirstOrDefault(v=>v&&v.ObjectId==objectId);if(!original)throw new Exception("The item to rebuild is unavailable");if(jobs.Values.Any(j=>j.id!=id&&j.objectId==objectId))throw new Exception("This item is already being rebuilt");captureTarget=original.transform.parent.TransformPoint(original.HomePosition);captureNormal=original.transform.parent.forward;}
    ...
    var job=AddJob(id,objectId,refine,capture,captureTarget);job.capture=capture;job.stage="Reading target";Status="Started scan "+job.number+" • "+jobs.Count+" running";
    Bridge.Send(new JObject{{"type","reconstruct"},{"epoch",generationEpoch},{"request_id",id},{"object_id",objectId},{"mode",refine?"refine":"new"},{"hint",hint},{"target",new JArray(viewport.x,1-viewport.y)},{"distance",Vector3.Distance(captureTarget,pose.position)},{"image","data:image/jpeg;base64,"+Convert.ToBase64String(data)}});
  ...
  void OnMessage(JObject e){
   ...
   case "reconstruction.cancelled":generationEpoch=Mathf.Max(generationEpoch,(int?)e["epoch"]??0);if((bool?)e["all"]==true){foreach(var old in jobs.Values.Where(j=>j.epoch<generationEpoch).ToArray()){cancelledJobs.Add(old.id);RemoveJob(old.id);}}if(e["request_ids"] is JArray cancelled){foreach(var id in cancelled){cancelledJobs.Add((string)id);RemoveJob((string)id);}}else if(!string.IsNullOrEmpty((string)e["request_id"]))RemoveJob((string)e["request_id"]);else ClearJobs();Status="Reconstruction cancelled";break;
   case "capture.request":Capture((string)e["request_id"],false,(string)e["hint"]??"");break;
   case "rebuild.request":Capture((string)e["request_id"],true,(string)e["hint"]??"",objectId:(string)e["object_id"]);break;
   ...
   case "reconstruction.started":{
    string id=(string)e["request_id"];if(!jobs.ContainsKey(id)&&(string)e["mode"]=="refine"){
     ...
     try{if(!original)throw new Exception("Item to refine no longer exists");AddJob(id,original.ObjectId,true,null,original.transform.parent.TransformPoint(original.HomePosition));}
   ...
   case "command":var result=Command(e);SyncScene();Bridge.Send(new JObject{{"type","command.result"},{"call_id",e["call_id"]},{"action",e["action"]},{"ok",result.ok},{"message",result.message}});SyncScene();break;
