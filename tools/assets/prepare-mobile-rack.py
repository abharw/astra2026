...
Run with ordinary Python; it launches Blender's bundled bpy + OpenUSD runtime:
  python3 tools/assets/prepare-mobile-rack.py --blender /Applications/Blender.app/Contents/MacOS/Blender
...
def arguments():
    ...
    parser.add_argument("--python-deps", type=Path, default=ROOT / ".local/processed-assets/python-deps", help="Directory containing pinned fast-simplification==0.1.13 (see processing README).")
    parser.add_argument("--input", type=Path, default=ROOT / ".local/asset-source/datacenter-rack/models/lazy/rack-exterior.usdz")
    ...
    if args.output is None:
        args.output = ROOT / (".local/processed-assets/rack-exterior-faithful-v2.usdz" if args.profile == "faithful" else ".local/processed-assets/rack-exterior-mobile.usdz")
    if args.evidence is None:
        args.evidence = ROOT / ("docs/evidence/mobile-rack-faithful-v2.json" if args.profile == "faithful" else "docs/evidence/mobile-rack-processing.json")
...
def run():
    ...
    evidence = {
        ...
        "source_correction_evidence": "docs/evidence/asset-intake.json and docs/evidence/rack-psu-offset-reproduction.py",
        "meshes": rows,
