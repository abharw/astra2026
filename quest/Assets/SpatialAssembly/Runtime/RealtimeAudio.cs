...
namespace SpatialAssembly {
 public class RealtimeAudio:MonoBehaviour {
  ...
  public void Stop(bool notify=true){ClearPlayback();awaitingPermission=false;starting=false;Enabled=false;if(mic){Microphone.End(null);Destroy(mic);mic=null;}lock(audioLock)samples.Clear();if(notify&&Bridge)Bridge.Send(new JObject{{"type","voice.stop"}});Status="Voice off";}
