...
import {detailConflict,detailInstructions,mergeDetail} from './detail.mjs';

const properties = p => ({type:'object',properties:p,required:Object.keys(p),additionalProperties:false});
const tools = [
 {type:'function',name:'load_asset_detail',description:'Load approved source-derived rack parts, available from the beginning through availableAssetDetails. Use for show the processor/memory/fans/etc in a server without loading everything first. Selects and focuses the loaded part after native acknowledgment. This loads authored assets, not generated geometry. Empty server uses the selected server. Choose all only for the whole server interior.',parameters:properties({server:{type:'string'},part:{type:'string',enum:['storage','fanwall','processors','heatsinks','network','power','chassis','motherboard','memory','all']}})},
 {type:'function',name:'unload_asset_detail',description:'Unload a loaded source rack part or all loaded internals of a selected server. Restores its exterior when no details remain. Keeps the rack and other servers.',parameters:properties({server:{type:'string'},part:{type:'string',enum:['storage','fanwall','processors','heatsinks','network','power','chassis','motherboard','memory','all']}})},
 {type:'function',name:'refine_part',description:'Generate deeper geometric and educational detail for a specific existing component, including its internal subcomponents. Runs in the background alongside other jobs. Other parts are preserved. Can deepen a generated subcomponent again. Use for go deeper, show inside this part, add more detail, or generate its inner mechanism. Empty part means the selected component.',parameters:properties({part:{type:'string'},instruction:{type:'string'}})},
...
export function startBridge({key,token,port=8796,host='127.0.0.1',model='gpt-6-astra',voiceModel='gpt-realtime-2.1',request,log=()=>{}}) {
 ...
 wss.on('connection',client=>{
  const sessionId=randomUUID();let upstream=null,voiceReady=false,latestScene=null,latestPointer=null,selectedPart='',activeObject='',viewRequest=null,requestCount=0,latestControlVersion=0,availableAssetDetails=[];
  ...
  const sceneContext=()=>({activeObject,availableAssetDetails,walkthrough:tour?{index:tour.index,total:tour.parts.length,part:tour.parts[tour.index]}:null,pointing:latestPointer,reconstructionJobs:[...jobs.values()].map(j=>({request_id:j.id,object_id:j.objectId,mode:j.mode,part:j.partId||null,stage:j.stage})),...(latestScene?{name:latestScene.name,selectedPart,parts:latestScene.parts.map(p=>({id:p.id,name:p.name,description:p.description,function:p.function,parentPartId:p.parentPartId||null,detailLevel:p.detailLevel||0,isInternal:!!p.isInternal,isHousing:!!p.isHousing,evidence:p.evidence,uncertainty:p.uncertainty,sourceIds:p.sourceIds})),sources:latestScene.research?.sources || [],uncertainty:latestScene.research?.gaps || []}:{})});
  ...
  function assetDetail(callId,args,unload=false){
   ...
   let server=String(args.server||selectedPart||'').split('.detail.')[0];
   if(!catalog.servers.includes(server)){toolResult(callId,{ok:false,error:'Select or name one of the available server IDs',servers:catalog.servers});return;}
   const part=String(args.part||'all');if(part!=='all'&&!catalog.groups?.some(g=>g.id===part)){toolResult(callId,{ok:false,error:'That source detail is unavailable',groups:catalog.groups});return;}
   queueCommand(callId,{action:unload?'unload_asset_detail':'load_asset_detail',object_id:catalog.object_id,server,part,amount:0});
  }
  ...
  function startVoice(){
   ...
   upstream.on('message',raw=>{let e;try{e=JSON.parse(raw);}catch{return;}switch(e.type){
    ...
    case 'response.function_call_arguments.done': {
     ...
     else if(e.name==='load_asset_detail'||e.name==='unload_asset_detail')assetDetail(e.call_id,args,e.name==='unload_asset_detail');
     else if(e.name==='reconstruct_target')captureForTool(e.call_id,'new',args);
     else if(e.name==='refine_part'){
      if(latestScene?.assetId==='arav-rack-v1'){toolResult(e.call_id,{ok:false,error:'Use load_asset_detail for approved rack parts. Finer source CAD is not registered; keep generated inference separate from this source asset.',availableAssetDetails});break;}
      stopTour();const q=String(args.part||selectedPart).toLowerCase();const part=q?latestScene?.parts.find(p=>p.id.toLowerCase()===q||p.name.toLowerCase().includes(q)):null;
      if(!part){toolResult(e.call_id,{ok:false,error:'Select or name the component to deepen',available:latestScene?.parts.map(p=>({id:p.id,name:p.name}))});break;}
      const instruction='Add deeper detail inside component '+part.name+' ('+part.id+'). '+String(args.instruction||'Explain its functional subcomponents');
      const old=captures.get(activeObject);
      if(old)void reconstruct({...old,request_id:randomUUID(),mode:'refine',object_id:activeObject,part_id:part.id,hint:instruction,call_id:e.call_id,epoch:generationEpoch});
      else captureForTool(e.call_id,'refine',{instruction,part_id:part.id});
     }
     else if(e.name==='refine_model'){
      if(latestScene?.assetId==='arav-rack-v1'){assetDetail(e.call_id,{server:selectedPart,part:'all'});break;}
  ...
  async function reconstruct(e){
   ...
   const partId=mode==='refine'?String(e.part_id||waiting?.args?.part_id||''):'';
   const previousAssembly=mode==='refine'?(assemblies.get(objectId)||(objectId===activeObject?latestScene:null)):null;
   if(jobs.has(request_id)||[...jobs.values()].some(j=>j.objectId===objectId&&(!previousAssembly||detailConflict(previousAssembly,partId,j.partId)))){send('reconstruction.error',{request_id,message:'This object already has a reconstruction in progress'});toolResult(callId,{ok:false,error:'Object already processing'});return;}
   if(mode==='refine'&&previousAssembly?.assetId==='arav-rack-v1'){send('reconstruction.error',{request_id,message:'Use approved asset detail loading for the authored rack'});toolResult(callId,{ok:false,error:'Authored rack geometry is preserved; use load_asset_detail'});return;}
   ...
   if(partId&&!previousAssembly?.parts.some(p=>p.id===partId)){send('reconstruction.error',{request_id,message:'Component unavailable'});toolResult(callId,{ok:false,error:'Component unavailable'});return;}
   const activeJob={id:request_id,objectId,partId,mode,epoch,stage:'starting',controller:new AbortController(),callId};jobs.set(request_id,activeJob);
   ...
   send('reconstruction.started',{request_id,epoch,mode,part_id:partId,object_id:objectId,max_concurrent:maxConcurrent});updateVoiceScene();
   ...
   try {
    ...
    const input=[{type:'input_text',text:JSON.stringify({target:e.target || [.5,.5],surfaceDistanceMeters:e.distance || null,request:e.hint || 'Reconstruct the pointed object',research,previousAssembly,detailRootId:partId||undefined})},{type:'input_image',image_url:e.image,detail:'high'}];
    const response=await api({model,store:false,reasoning:{effort:'medium'},max_output_tokens:24000,instructions:reconstructionInstructions+'\n'+referenceInstructions+(partId?'\n'+detailInstructions:''),input:[{role:'user',content:input}],text:{format:{type:'json_schema',name:'ar_assembly',strict:true,schema:assemblySchema}}},activeJob.controller.signal);
    if(activeJob.controller.signal.aborted || jobs.get(request_id)!==activeJob)throw Error('Cancelled');
    let assembly=groundAssembly(validateAssembly(JSON.parse(responseText(response))),research);
    if(partId)assembly=mergeDetail(assemblies.get(objectId)||previousAssembly,previousAssembly,partId,assembly,request_id);
    else{assembly.captureId=request_id;assembly.revision=(mode==='refine'?(previousAssembly?.revision || 1):0)+1;}
    ...
    updateVoiceScene();toolResult(callId,{ok:true,name:assembly.name,parts:assembly.parts.map(p=>({id:p.id,name:p.name,parentPartId:p.parentPartId||null})),refinedPart:partId||null,note:'Generated assembly returned to the device. Rendering/placement may still need confirmation; geometry is approximate.',research:research.status});
  ...
  client.on('message',raw=>{let e;try{e=JSON.parse(raw);}catch{return;}switch(e.type){
   ...
   case 'scene.update':
    try {if(Array.isArray(e.available_asset_details))availableAssetDetails=e.available_asset_details.slice(0,4);latestControlVersion=Number.isInteger(e.selection_version)?e.selection_version:latestControlVersion;if(tour&&tour.objectId!==String(e.object_id || ''))stopTour();latestScene=e.assembly?validateAssembly(e.assembly):null;activeObject=String(e.object_id || '');selectedPart=String(e.selected_part || '');latestPointer=e.pointing&&typeof e.pointing==='object'?e.pointing:null;if(latestScene&&activeObject){const cached=assemblies.get(activeObject);if(cached&&(cached.revision||1)>(latestScene.revision||1))latestScene=cached;else assemblies.set(activeObject,latestScene);}updateVoiceScene();}catch{send('scene.error',{message:'Invalid scene state'});}break;
   case 'part.explain': {
