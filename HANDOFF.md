...
The user requested independent `app/AR/` and `app/VR/` directories and explicitly asked to preserve both source branches. Integration work is on `codex/ar-vr-integration` in `/Users/aravb/Developer/astra2026-ar-vr`, based on Arav `d70f39f` and Akeil `98dfc61`. Do not commit this integration to `Arav`, `Akeil`, or `main`. The original Arav checkout is unchanged.
